...
function Markdown({ text, onPaperLink }) {
  const html = useMemo(() => DOMPurify.sanitize(marked.parse(text || '', { breaks: true, gfm: true }), { ALLOWED_URI_REGEXP: /^(?:(?:https?|mailto|paperdesk):|[^a-z]|[a-z+.-]+(?:[^a-z+.-:]|$))/i }), [text]);
...
function Workspace({ auth, onAuthChange }) {
  ...
  currentNoteRef.current = { id: selectedId, text: noteText, base: noteBaseline, dirty: noteDirty, loading: noteLoading };
  ...
  useEffect(() => {
    ...
    api(`/api/papers/${encodeURIComponent(selectedId)}/note`).then(data => { if (!cancelled) { setNoteText(data.markdown || ''); setNoteBaseline(data.markdown || ''); } }).catch(e => { if (!cancelled) setNoteError(e.message); }).finally(() => { if (!cancelled) setNoteLoading(false); });
  ...
  const saveNote = useCallback(async (quiet = false) => {
    ...
    if (!current.id || !current.dirty || current.loading) return true;
    setNoteSaving(true);
    const operation = (async () => {
      try { const saved = await api(`/api/papers/${encodeURIComponent(current.id)}/note`, { method: 'PUT', body: { markdown: current.text, baseMarkdown:current.base } }); if (selectedIdRef.current === current.id) { const baseline = saved.markdown ?? current.text; const text = saved.conflict && currentNoteRef.current.text === current.text ? baseline : currentNoteRef.current.text; currentNoteRef.current = {...currentNoteRef.current,base:baseline,text,dirty:text!==baseline}; setNoteBaseline(baseline); if(saved.conflict && text === baseline) setNoteText(baseline); } if(saved.conflict) notify('另一端也修改了笔记，两份内容均已保留，请整理后保存。','error'); setPapers(prev => prev.map(p => p.id === current.id ? { ...p, hasNote: !!current.text.trim(), updatedAt: new Date().toISOString() } : p)); refreshSync(); if (!quiet && !saved.conflict) notify('笔记已保存'); return true; }
      catch (e) { notify(`笔记未能保存：${e.message}`, 'error'); return false; }
